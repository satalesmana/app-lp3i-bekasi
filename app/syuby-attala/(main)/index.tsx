...
import { Button, FlatList, KeyboardAvoidingView, Platform, SafeAreaView, TextInput, View } from 'react-native';
...
export default function MainScreen() {
  ...
  return (
    <KeyboardAvoidingView style={{ display: 'flex', flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 0 : 24}>
      <SafeAreaView style={{ flex: 1, padding: 16 }}>
        <FlatList data={messages} keyExtractor={(item) => item.id.toString()} inverted renderItem={({ item }) => <MessageItem item={item} />} contentContainerStyle={{ paddingVertical: 10 }} />
